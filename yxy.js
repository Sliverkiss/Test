...
// env.js 全局
const $ = new Env("忆晓缘-广告");
const ckName = "yxy_data";
//-------------------- 一般不动变量区域 -------------------------------------
...
async function main() {
    ...
    for (let user of userList) {
        if (user.ckStatus) {
            ...
            taskall.push(await user.lookList());
...
class UserInfo {
    constructor(str) {
        ...
        this.headers = {
            ...
            "User-Agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 14_8 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MicroMessenger/8.0.41(0x18002930) NetType/WIFI Language/zh_CN"
        };
    }
    getRandomTime() {
    ...
    async look(ad) {
        try {
            const options = {
                //签到任务调用签到接口
                url: `https://d48mreez.xiaomy.net/clientApi/ad/look`,
                //请求头, 所有接口通用
                headers: {
                    "Content-Type": "application/json;charset=utf-8",
                    "platform": "MP-WEIXIN",
    ...
    async point() {
        try {
            const options = {
                //签到任务调用签到接口
                url: `https://d48mreez.xiaomy.net/clientApi/user/fhInfo`,
                //请求头, 所有接口通用
                headers: {
                    "Content-Type": "application/json;charset=utf-8",
                    "platform": "MP-WEIXIN",
            ...
            //post方法
            let result = await httpRequest(options);
            DoubleLog(`【${result?.data?.fenHongGu?.userId}】分红：${result?.data?.fenHongGu?.fhg}`)
        } catch (e) {
            console.log(e);
        }
    }
