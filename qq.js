...
class UserInfo {
    constructor(str) {
        ...
        this.token=this.ckInfo[0];
        this.uid=this.ckInfo[1];
        this.ckStatus = true
    ...
    async signin() {
        try {
            ...
            if (result?.status == 0) {
                //obj.error是0代表完成
                DoubleLog(`✅签到成功！获取${result?.data?.value}积分`);
            } else {
    ...
    async taskList() {
        try {
            do{
                ...
                await this.finishViewPage();
                await this.drawLottery();
            }while(this.taskStatus);
        } catch (e) {
            console.log(e);
        }
    }
    //浏览页面
    async finishViewPage() {
        try {
            const options = {
                //签到任务调用签到接口
    ...
    async drawLottery() {
        try {
            ...
            let result = await httpRequest(options);
            if (result?.status == 0) {
                console.log(`✅抽奖成功！获得${result?.data?.awardOrderList[0].awardName}`)
            } else {
                //抽奖次数达到上限，停止抽奖
                DoubleLog(`✅${result?.msg}`);
    ...
    async point() {
        try {
            ...
            let result = await httpRequest(options);
            if (result?.status == 0) {
                DoubleLog(`✅目前总共${result?.data?.point}积分`)
            } else {
                //抽奖次数达到上限，停止抽奖
                console.log(result);
            }
...
async function getCookie() {
    if ($request && $request.method != 'OPTIONS') {
        ...
        console.log(bodyValue['uid']); 
        console.log(bodyValue)
        if (tokenValue&&bodyValue) {
            // const ck=tokenValue+"#"+bodyValue;
            // $.setdata(ck,ckName);
