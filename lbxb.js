...
脚本名称：微信小程序 立白小白白会员俱乐部 签到、评论、答题
脚本作者：@Sliverkiss
更新日期：2023.08.30 11:04:11
...
1.抓取clubwx.hm.liby.com.cn域名下的unionId和X-wxde54fd27cb59db51-Token
2.进入小程序的答题页面，抓取clubwx.hm.liby.com.cn域名下的Cookie
3.将unionId、X-wxde54fd27cb59db51-Token、Cookie分别用#连接，如unionId#X-wxde54fd27cb59db51-Token#Cookie，填写到lbxb_data，多账号用@分割
...
async function main() {
    ...
    for (let user of userList) {
        if (user.ckStatus) {
            ...
            taskall.push(await user.getDaTi());
            await $.wait(user.getRandomTime());
            taskall.push(await user.right());
            await $.wait(user.getRandomTime());
            taskall.push(await user.savePostComment());
            await $.wait(user.getRandomTime());
            taskall.push(await user.getTaskReward1());
            await $.wait(user.getRandomTime());
            taskall.push(await user.getTaskReward2());
            await $.wait(user.getRandomTime());
...
class UserInfo {
    constructor(str) {
        this.index = ++userIdx;
        this.ck = str.split('#');
        this.userId = this.ck[0];
        this.token=this.ck[1];
        this.cookie=this.ck[2];
        this.ckStatus = true
        this.headers = {
    ...
    async signin() {
        try {
            const options = {
                //签到任务调用签到接口
                url: `https://clubwx.hm.liby.com.cn/miniprogram/benefits/activity/sign/execute.htm?taskId=158`,
                //请求头, 所有接口通用
    ...
    async getDaTi() {
        try {
            const options = {
                //签到任务调用签到接口
                url: `https://clubwx.hm.liby.com.cn/me/getDaTi.htm?num=1&platformCode=XiaoBaiBai`,
                //请求头, 所有接口通用
                headers: {
                    "Cookie":this.cookie
                },
            ...
            switch (data.titles) {
                case 1:
                    this.question=data.question1;
                    this.answer=data.answer2;
                    break;
                case 2:
                    this.question=data.question2;
                    this.answer=data.answer1;
                    break;
            }
    ...
    async right() {
        try {
            let enAnswer=encodeURIComponent(this.answer);
            const options = {
                //签到任务调用签到接口
                url: `https://clubwx.hm.liby.com.cn/me/right.htm?answer=${enAnswer}&titleNum=1&option=${enAnswer}&platformCode=XiaoBaiBai`,
                //请求头, 所有接口通用
                headers: {
                    "Cookie":this.cookie
                },
        ...
        try {
            const options = {
                ...
                body:{
                  ...
                  "integral" : "2",
                  "title" : "评论文章",
                  "articleConnects" : "\/pages\/subPackages\/bbs\/pages\/detail\/index?id=420378",
                  "rewardTimes" : 0,
                  "taskType" : 2,
                  "chooseType" : 0,
                  "id" : 66,
                  "connectsId" : 420378,
                  "subscriptionSwitch" : 1,
                  "appId" : "1",
                  "isComplete" : 1,
                  "suitableScope" : 0,
                  "showCase" : "0,1"
            ...
            if (result?.code == 200) {
                ...
                DoubleLog(`🔶${options.body.title}:${result?.msg}`)
                console.log(result);
            }
        } catch (e) {
            console.log(e);
        ...
        try {
            const options = {
                ...
                body:{
                  ...
                  "integral" : "9",
                  "title" : "看好戏 每天送积分",
                  "articleConnects" : "\/pages\/subPackages\/bbs\/pages\/detail\/index?id=422292",
                  "describes" : "参与评论奖励9积分",
                  "rewardTimes" : 0,
                  "taskType" : 2,
                  "chooseType" : 0,
...
async function getNotice() {
    try{
        const urls=["https://ghproxy.com/https://raw.githubusercontent.com/Sliverkiss/GoodNight/main/notice.json","https://ghproxy.com/https://raw.githubusercontent.com/Sliverkiss/GoodNight/main/tip.json"];
